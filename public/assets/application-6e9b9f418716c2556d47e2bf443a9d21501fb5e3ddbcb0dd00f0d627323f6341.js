...
%o`, message, error2, detail);
  ...
  var form_controller_default = class extends Controller {
    ...
    remove_form_validation() {
      this.disable_phone_validator();
    ...
    enable_required_validator() {
      this.form.querySelectorAll(".required").forEach((el) => {
        ...
        if (el.type === "hidden") {
          el.addEventListener("change", this._form_validator_by_required);
        }
      });
      this.form.querySelectorAll(".email").forEach((el) => {
        if (!el.className.includes("required")) {
          el.addEventListener("input", this._form_validator_by_required);
        }
      });
    }
    // enable_phone_validator() {
    //   this.form.querySelectorAll('.phone-number').forEach(phone_field => {
    //     if (phone_field.tagName === 'INPUT') {
    //       phone_field.addEventListener('input', this._phone_validator);
    //     }
    //   })
    // }
    // disable_phone_validator() {
    //   this.form.querySelectorAll('.phone-number').forEach(phone_field => {
    //     if (phone_field.tagName === 'INPUT') {
    //       phone_field.removeEventListener('input', this._phone_validator);
    //     }
    //   })
    // }
    // _phone_validator(e) {
    //   const phone_number_format = e.target.getAttribute('phone-number-format') || "xxx-xxx-xxxx";
    //   const phone_formats = phone_number_format.trim().replace('(', '').replace(')', '').split('-');
    //   if (phone_formats.length > 0) {
    //     const regexs = [];
    //     phone_formats.forEach(format=>{
    //       regexs.push(`(\\d{0,${format.length}})`);
    //     });
    //     var x = e.target.value.replace(/\D/g, '').match(new RegExp(regexs.join('')));
    //     if (phone_formats[0].includes('x')) {
    //       const p_numbers = []
    //       phone_formats.forEach((v, index) => {
    //         if (x[index + 1]) {
    //           p_numbers.push(x[index + 1])
    //         }
    //       });
    //       e.target.value = p_numbers.join('-');
    //     } else {
    //       const country_code_length = phone_formats[0].length;
    //       let matched_country_code = false;
    //       for(let i = 0; i < country_code_length; i++) {
    //         if (phone_formats[0].substring(0, i + 1) === x[1] ) {
    //           matched_country_code = true;
    //           break;
    //         }
    //       }
    ...
    //   }
    // }
    _form_validator_by_required(e) {
      let disabled = false;
      this.form.querySelectorAll(".required").forEach((el) => {
        setup_valid_element(el, true);
        if (["INPUT", "SELECT", "TEXTAREA"].includes(el.tagName)) {
          if (el.tagName === "INPUT") {
            if (this.form[el.name].value === null || this.form[el.name].value === "") {
              disabled = true;
              setup_valid_element(el, false);
              return;
            }
          } else {
        ...
        if (el.className.includes("email")) {
          const error_desc_element = el.parentElement.querySelector('[data-id="email_invalid_error"]');
          if (!_is_valid_email(el.value)) {
            if (error_desc_element)
              error_desc_element.classList.remove("hidden");
            disabled = true;
            setup_valid_element(el, false);
            return;
          } else {
            if (error_desc_element)
              error_desc_element.classList.add("hidden");
          }
        }
      });
      this.form.querySelectorAll(".email").forEach((el) => {
        ...
        if (["INPUT"].includes(el.tagName) && el.value.length > 0) {
          if (!_is_valid_email(el.value)) {
            disabled = true;
            setup_valid_element(el, false);
            return;
      ...
      this.form.querySelectorAll(".require-form-validation").forEach((el) => {
        ...
        if (el.getAttribute("data-target")) {
          set_check_target_element(el.getAttribute("data-target"));
        }
        if (el.getAttribute("data-valid")) {
          document.getElementById(el.getAttribute("data-valid")).value = disabled ? "0" : "1";
        }
      });
    }
  };
  ...
  function setup_valid_element(el, isValid) {
    if (el) {
      if (isValid) {
