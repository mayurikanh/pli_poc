...
%o`, message, error2, detail);
  ...
  var form_controller_default = class extends Controller {
    ...
    enable_required_validation() {
      ...
      var employer_end_date = this.form.querySelector(".emplyment_end_date");
    ...
    _form_validation(e) {
      ...
      const email_address = this.form.querySelector(".email");
      setup_valid_element(email_address, true);
      if (email_address.className.includes("hidden")) {
        return;
      }
      if (["INPUT"].includes(email_address.tagName) && email_address.value.length > 0) {
        if (!_is_valid_email(email_address.value)) {
          disabled = true;
          setup_valid_element(email_address, false);
          return;
        }
      }
