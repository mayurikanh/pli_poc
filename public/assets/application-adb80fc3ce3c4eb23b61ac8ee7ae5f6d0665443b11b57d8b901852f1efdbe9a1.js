...
%o`, message, error2, detail);
  ...
  var form_controller_default = class extends Controller {
    ...
    add_form_validation() {
      this.enable_required_validation();
      this.enable_phone_validation();
    ...
    enable_required_validation() {
      this.form.querySelectorAll(".required").forEach((el) => {
        if (el) {
          ...
          if (el.type === "hidden") {
            el.addEventListener("change", this._form_validation);
          }
        }
      });
      this.phone_number = this.form.querySelector(".phone-number");
      if (phone_number) {
        if (!phone_number.className.includes("required")) {
          phone_number.addEventListener("input", this._form_validation);
        }
      }
      this.email_address = this.form.querySelector(".email");
      if (email_address) {
        if (!email_address.className.includes("required")) {
          email_address.addEventListener("input", this._form_validation);
        }
      }
      this.employer = this.form.querySelector(".employer");
      if (employer) {
        if (!employer.className.includes("required")) {
          employer.addEventListener("input", this._form_validation);
        }
      }
      this.employer_start_date = this.form.querySelector(".start_date");
      if (employer_start_date) {
        if (!employer_start_date.className.includes("required")) {
          employer_start_date.addEventListener("input", this._form_validation);
        }
      }
      this.employer_end_date = this.form.querySelector(".emplyment_end_date");
      if (employer_end_date) {
        if (!employer_end_date.className.includes("required")) {
          employer_end_date.addEventListener("input", this._form_validation);
        }
      }
    }
    // disable_required_validation() {
    //   this.form.querySelectorAll('.required').forEach(el => {
    //     if (['INPUT'].includes(el.tagName)) {
    //       el.removeEventListener('input', this._form_validation);
    //     }
    //     if (el.type === 'hidden') {
    //       el.removeEventListener('change', this._form_validation);
    //     }
    //   })
    //   this.form.querySelectorAll('.phone-number').forEach(el => {
    ...
    //     }
    //   })
    //   this.form.querySelectorAll('.employer').forEach(el => {
    //     if (!el.className.includes('required')) {
    //       el.removeEventListener('input', this._form_validation);
    //     }
    //   })
    //   this.form.querySelectorAll('.start_date').forEach(el => {
    //     if (!el.className.includes('required')) {
    //       el.removeEventListener('input', this._form_validation);
    //     }
    //   })
    //   this.form.querySelectorAll('.emplyment_end_date').forEach(el => {
    //     if (!el.className.includes('required')) {
    //       el.removeEventListener('input', this._form_validation);
    //     }
    //   })
    ...
    _form_validation(e) {
      ...
      const email_address2 = this.form.querySelector(".email");
      setup_valid_element(email_address2, true);
      if (email_address2.className.includes("hidden")) {
        return;
      }
      if (["INPUT"].includes(email_address2.tagName) && email_address2.value.length > 0) {
        if (!_is_valid_email(email_address2.value)) {
          disabled = true;
          setup_valid_element(email_address2, false);
          return;
        }
      }
